...
% Last Modified by GUIDE v2.5 07-Feb-2025 08:59:34
...
versionName='EDF/EDF+ gui v4.0 @2025YP';        %Updated 2025-2-7
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
runOneSegmentSpectrogram();

function runOneSegmentSpectrogram()
info=getappdata(0,'info');
...
detectOneSegmentState();

function detectOneSegmentState()
info=getappdata(0,'info');
...
elseif seizureMethod==2
	state = getState2(pDat,mDat.Amp,mode);
...
if anaMode(1)
    ...
    %figure for seizure
    plotData2(pDat,mDat,state,fname);
end
...
% --- Executes on button press in pushbutton_file_runall.
function pushbutton_file_runall_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_file_runall (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
info=getappdata(0,'info');
pnum=round(info.totalHour);
procWindow=[0,60];
for i=1:pnum
    ...
    procWindow=procWindow+60;
end
disp('Processing the entire session is done!');


function saveOneSegment()
PathName=getappdata(0,'PathName');
specDat=getappdata(0,'specDat');
